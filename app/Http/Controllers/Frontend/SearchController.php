...
class SearchController extends Controller
{
    ...
    {
        if (!$req->ajax()) {
            $movie = Movie::where('name','like','%'.$req->keyword.'%')->paginate(1);
            $tukhoa = $req->keyword;
            // phân trang
        ...
        $top_view_all = Movie::orderby('view_count','desc')->limit(10)->get();
         return view('frontend.pages.search',compact('movie','to','from','top_view_all','top_view_this_day','top_view_this_Week','top_view_this_Month','tukhoa'));
        }else{
            $movie = Movie::where('name','like','%'.$req->keyword.'%')->get();
            $keyword = '<li>Kết quả cho từ khóa: <strong style="color: red;">'.$req->keyword.'</strong></li>';
            if ($movie->count()>=3) {
                $movie = Movie::where('name','like','%'.$req->keyword.'%')->limit(3)->get();
            }else{
                $movie = Movie::where('name','like','%'.$req->keyword.'%')->limit(2)->get();
            }
            $search = '';
            foreach($movie as $key => $value){
                $search .= '<li class="exact_result">
                                <a href="'.route("movie",$value->slug).'">
                                    <div class="halim_list_item">
                                       <div class="image"><img src="'.url('public/uploads').'/'.$value->image.'" alt="halimthemes.com"></div>
                                        <span class="label">'.$value->name.'</span>
                                        <span class="enName">'.$value->substitute_name.'</span>
                                    </div>
                                </a>
                            </li>';
            }
