...
class NewMovieController extends Controller
{
    ...
    {
        $movie = Episode::join('movies','movies.id','=','episodes.id_movie')->select(array(DB::raw('max(episodes.created_at) as time'),'movies.*'))->orderby('time','desc')->groupBy('episodes.id_movie')->paginate(1);
        ...
        return view('frontend.pages.new_movie',compact('movie','to','from','top_view_all','top_view_this_day','top_view_this_Week','top_view_this_Month'));
    }
}
