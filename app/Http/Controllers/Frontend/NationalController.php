...
{
    public function national($slug)
    {
        $national = National::where('slug',$slug)->first();
        $name_national = $national->name;
        $movie = $national->national_movie()->paginate(1);
        ...
        $top_view_all = Movie::orderby('view_count','desc')->limit(10)->get();
         return view('frontend.pages.national',compact('movie','to','from','top_view_all','top_view_this_day','top_view_this_Week','top_view_this_Month','name_national'));
    }
}
