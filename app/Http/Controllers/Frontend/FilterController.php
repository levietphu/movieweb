...
class FilterController extends Controller
{
    ...
    {
        $movie = Movie::join('cate_movie','cate_movie.id_movie','=','movies.id')->join('categories','categories.id','=','cate_movie.id_cate')->sort($req)->type($req)->status($req)->national($req)->release($req)->cate($req)->groupBy('movies.id')->select('movies.*')->paginate(1);
        ...
        return view('frontend.pages.filter',compact('movie','to','from','top_view_all','top_view_this_day','top_view_this_Week','top_view_this_Month'));
    }
}
